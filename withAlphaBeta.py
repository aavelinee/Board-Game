...
class MinimaxPlayer(Game, Player):
	...
	def playingTaws(self, moves):
		seen = []
		for i in moves:
			if [i[0],i[1]] not in seen:
				seen.append([i[0], i[1]])
		return len(seen)

	def evalFunc(self, selfMoves, selfMoveSize, opponentMoves, opponentMovesSize):
		selfTaws = self.playingTaws(selfMoves)
		opponentTaws = self.playingTaws(opponentMoves)
		return (6*float(selfMoveSize) + float(selfTaws)) - (float(opponentMovesSize) + float(opponentTaws))

	def minValue(self, board, depth, alpha, beta):
		finalMove = []
		value, beta = 1000, 1000
		
		selfMoves = self.generateMoves(board, self.opponent(self.side))
		opponentMoves = self.generateMoves(board, self.side)


		selfMoveSize = len(selfMoves)
		opponentMovesSize = len(opponentMoves)

		if depth == self.depth:
			finalVal = self.evalFunc(selfMoves, selfMoveSize, opponentMoves, opponentMovesSize)
			return finalMove, finalVal

		for move in selfMoves:
			nextBoard = self.nextBoard(board, self.opponent(self.side), move)

			depth += 1
			maxMove, maxVal = self.maxValue(nextBoard, depth, alpha, beta)
			depth -= 1
	...
	def maxValue(self, board, depth, alpha, beta):
		...
		selfMoves = self.generateMoves(board, self.side)
		...
		for move in selfMoves:
			...
			depth += 1
			minMove, minVal = self.minValue(nextBoard, depth, alpha, beta)
			depth -= 1
	...
	def getMove(self, board):
		...
		alpha = -1000
		beta = 1000
		move, value = self.maxValue(board, depth, alpha, beta)
		return move
...
if __name__ == '__main__':
	gameSize = 8
	game = Game(gameSize)
	# human1 = HumanPlayer(gameSize)
	# human1 = RandomPlayer(gameSize)
	human1 = MinimaxPlayer(gameSize)
	human1.initialize('B', 3)
	# human2 = HumanPlayer(gameSize)
	# human2 = RandomPlayer(gameSize)
	human2 = MinimaxPlayer(gameSize)
	human2.initialize('W', 4)
